...
export default class DataTableUtils {
  ...
  reloadDataTable(isReset, data, firstLoad, currentState) {
    ...
    if(dataObj.keyword){
     let arrayObj = [];
      dataObj.value=dataObj.keyword
      dataObj.key=this.searchKey
      console.log('dataobject----', dataObj)
       arrayObj.push(dataObj)
      NavigateTable(this.searchUrl, arrayObj, this.makeCallBack, (response) => this.showData(response.data, data), this.loadingFunction, { ...this.SEARCH_API_Options, firstLoad });
 
    }else{
      NavigateTable(this.listUrl, dataObj, this.makeCallBack, (response) => this.showData(response.data, data), this.loadingFunction, { ...this.API_Options, firstLoad });
  ...
  getRequestData(isReset, data, currentState = {}) {
    ...
    if (data && data.isDownload) {
      searchParams = {
        "pageNo": 1 - 1 ,
        "pageSize": (currentState.totalRecords ? currentState.totalRecords : ""),
      ...
      searchParams = {
        "pageNo": (isReset === true ? 1 - 1 : ((data && data.pageNo) ? data.pageNo - 1 : currentState.pageNo - 1)) || "",
        "pageSize": (data && data.rowCount ? data.rowCount : currentState.rowCount) || "",
        "sort": (data && data.orderByCol ? data.orderByCol : currentState.orderByCol) || "",
        "order": (data && data.sort ? data.sort : currentState.sort) || "",
        "keyword": (data && data.keyword ? data.keyword : currentState.keyword),
        ...filters
  ...
  showData(response, currentState) {
    ...
    if (this.postRequestCallback && _.isFunction(this.postRequestCallback)) {
      ...
      const responseData = {
        ...
        "pageNo": response.number + 1,
        // "rowData": rowDataList
        "rowData":this.isInfiniteScroll? this.rowDataArr:rowDataList
      }
      const { totalRecords, rowCount, rowData, pageNo } = responseData;
      const startRow = (rowCount * (pageNo - 1)) + 1;
